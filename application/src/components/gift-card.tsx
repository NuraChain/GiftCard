...
export default function GiftCard({ tier }: { tier: CatalogTier }): ReactNode
{
    ...
    const pay = async (): Promise<void> =>
    {
        ...
        {
            setPaying(false);
            setFailure(failureText(error, 'شروع پرداخت ممکن نشد. اتصال اینترنت را بررسی کنید و دوباره تلاش کنید.'));
        }
    };
    ...
    return (
        <article
            className={ `flex flex-col rounded-2xl border bg-surface p-5 sm:p-6 ${
                tier.recommended ? 'border-firouze shadow-[0_1px_24px_-8px_var(--firouze)]' : 'border-line'
            }` }
        >
            ...
            <p className="mt-3 text-start text-3xl font-bold sm:text-4xl">
                <span dir="ltr" className="latin inline-block">${ tier.amount }</span>
            </p>
                ...
                { step === 'phone' ? (
                    ...
                    >
                        <Field
                            label="شماره موبایل"
                            htmlFor={ inputId }
                            error={ form.errorFor('phone') === '' ? '' : 'شماره موبایل معتبر نیست. مثل ۰۹۱۷۰۴۵۹۳۳۰ بنویسید.' }
                            hint={ form.errorFor('phone') === '' ? 'کد به همین شماره پیامک می‌شود.' : undefined }
                        >
                            <TextInput
                                id={ inputId }
                                type="tel"
                        ...
                        <Button type="submit" variant={ tier.recommended ? 'primary' : 'outline' } full className="mt-4" disabled={ soldOut }>
                    ...
                    <div className="mt-3">
                        <div className="rounded-xl border border-line bg-paper p-4">
                            ...
                            <p dir="ltr" className="latin mt-1 text-start text-lg font-semibold">{ form.values.phone }</p>
                            <button
                                type="button"
                        ...
                        <dl className="mt-4 grid gap-2 text-small">
                            <div className="flex justify-between gap-2">
                                ...
                                <dd className="min-w-0 text-end font-bold">{ toman(tier.toman) } تومان</dd>
                            </div>
                        </dl>
