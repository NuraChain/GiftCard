...
export default function InventoryTable({ tiers, revision }: InventoryTableProps): ReactNode
{
    ...
    const query = useRef<{ search: string; state?: CodeState; amount?: number; page: number }>({ search: '', page: 1 });
    ...
    const applyFilter = (next: { state?: CodeState | null; amount?: number | null; search?: string }): void =>
    {
    ...
    return (
        <section className="mt-section">
            ...
            <form
                ...
                noValidate
                onSubmit={ (event: FormEvent) => { event.preventDefault(); applyFilter({}); } }
            >
                <label className="sr-only" htmlFor="code-search">جستجو در کدها</label>
            ...
            <div className="mt-3 flex gap-2">
                <Select
                    ...
                    onChange={ (chosen) => applyFilter({ state: chosen === '' ? null : chosen as CodeState }) }
                />
                <Select
                    id="filter-amount"
                    ...
                    onChange={ (chosen) => applyFilter({ amount: chosen === '' ? null : Number(chosen) }) }
                />
            </div>
                            ...
                            <tbody>
                                { rows.map((row) => (
                                    <tr key={ row.code } className="border-b border-line/60">
                                        <td data-label="کد" className="cell-action p-3 whitespace-nowrap max-md:order-6">
                                            { revealed === row.code ? (
                                                <button
                                                    type="button"
                                                ...
                                                >
                                                    <span dir="ltr" className="latin break-all">{ row.code }</span>
                                                    <Copy className="size-3.5 shrink-0 text-muted" aria-hidden="true"/>
                                                </button>
                                            ) : (
                                                <button
                                                    type="button"
                                        ...
                                        <td data-label="کارت" className="cell-inline p-3 whitespace-nowrap max-md:order-2">
                                            <span dir="ltr" className="latin">${ row.amount }</span>
                                        </td>
                                        <td data-label="وضعیت" className={ `cell-inline p-3 whitespace-nowrap font-bold max-md:order-1 ${ STATE_TEXT[row.state].tone }` }>
                                            { STATE_TEXT[row.state].label }
                                        </td>
                                        <td data-label="خریدار" className="cell-inline p-3 whitespace-nowrap max-md:order-3">
                                            { row.phone === null
                                                ? <span className="text-muted">-</span>
                                                : <span dir="ltr" className="latin">{ row.phone }</span> }
                                        </td>
                                        <td data-label="افزوده شد" className="cell-inline p-3 whitespace-nowrap text-caption text-muted max-md:order-4">{ moment(row.addedAt) }</td>
                                        <td data-label="فروخته شد" className="cell-inline p-3 whitespace-nowrap text-caption text-muted max-md:order-5">
