    ...
    return (
        <section>
            <div className="flex flex-wrap items-center gap-3">
                ...
                <Button glyph={ Plus } compact className="ms-auto" onClick={ () => setDraft(emptyTier((tiers.length + 1) * 10)) }>
                    کارت تازه
                </Button>
            </div>
            ...
            <div className="mt-4">
                <Async
                    ...
                    onRetry={ () => void load() }
                    skeleton={ <div className="anim-pulse h-40 rounded-2xl border border-line bg-surface"></div> }
                >
                        ...
                        { tiers.map((tier) => (
                            <li
                                ...
                                className={ `rounded-2xl border p-4 sm:flex sm:flex-wrap sm:items-center sm:gap-3 ${
                                    tier.active ? 'border-line bg-surface' : 'border-line/60 bg-surface/50'
                                }` }
                            >
                                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 sm:contents">
                                    <span dir="ltr" className="latin text-lg font-bold">${ tier.amount }</span>
                                    <span className="text-small">{ tier.title }</span>
                                    <span className="text-small text-muted">{ toman(tier.toman) } تومان</span>
                                    { tier.recommended && (
                                        <span className="rounded-full bg-firouze/15 px-2.5 py-0.5 text-caption font-bold text-firouze">پیشنهادی</span>
                                    ) }
                                    { !tier.active && (
                                        <span className="rounded-full bg-muted/15 px-2.5 py-0.5 text-caption font-bold text-muted">غیرفعال</span>
                                    ) }
                                </div>

                                <div className="mt-3 flex gap-2 sm:mt-0 sm:ms-auto">
                                    <Button size="sm" grow onClick={ () => setDraft({ ...tier }) }>ویرایش</Button>
            ...
            { draft !== null && (
                <form className="mt-4 rounded-2xl border border-firouze bg-surface p-5" noValidate onSubmit={ (event) => void saveTier(event) }>
                    <h3 className="font-bold">
                        { draft.amount > 0
                            ? <span>ویرایش کارت <span dir="ltr" className="latin">${ draft.amount }</span></span>
                            : <span>کارت تازه</span> }
                    </h3>
                    ...
                    <label className="mt-4 block text-small font-bold" htmlFor="tier-blurb">توضیح روی کارت</label>
                    <textarea
                        id="tier-blurb"
                        rows={ 3 }
