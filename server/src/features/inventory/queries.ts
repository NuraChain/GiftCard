...
export function createCodeQueries(db: DatabaseSync): CodeQueries {
    const insertCode = db.prepare(
        'INSERT OR IGNORE INTO codes (amount, code, added_at) VALUES (?, ?, ?)'
    );
    ...
    const countAvailable = db.prepare(
        'SELECT COUNT(*) AS n FROM codes WHERE amount = ? AND order_id IS NULL'
    );
    ...
    const releaseStatement = db.prepare(
        'UPDATE codes SET order_id = NULL, hold_until = NULL WHERE order_id = ?'
    );
    ...
    const expireHolds = db.prepare(
        'UPDATE codes SET order_id = NULL, hold_until = NULL WHERE hold_until IS NOT NULL AND hold_until < ?'
    );
    ...
    return {
        ...
        stock(tiers) {
            sweep();
            const counted = shaped<
                Array<{ amount: number; available: number; held: number; sold: number }>
            >(countStock.all());
