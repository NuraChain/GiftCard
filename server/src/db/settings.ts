...
export function createSettingQueries(db: DatabaseSync): SettingsStore
{
    ...
    const insertSettingLog = db.prepare('INSERT INTO settings_log (key, before, after, changed_at) VALUES (?, ?, ?, ?)');
    const selectSettingLog = db.prepare('SELECT key, before, after, changed_at FROM settings_log ORDER BY id DESC LIMIT ?');
    ...
    settingsLog(limit)
    {
        return shaped<Array<{ key: string; before: string; after: string; changed_at: string }>>(selectSettingLog.all(limit))
            .map((row) => ({ key: row.key, before: row.before, after: row.after, changedAt: row.changed_at }));
    }
    };
}
