    ...
    const state: Fakes = {
        ...
        payment: {
            request: (input) =>
            {
                ...
                return Promise.resolve({ ok: true, authority, payUrl: `https://gateway.test/pay/${ authority }` });
            },
...
beforeEach(() =>
{
    ...
    app = buildApp({
        ...
        sms: fake.sms,
        admin: createAdmin({ matches: (candidate) => settings.matchesAdminKey(candidate), secureCookie: false }),
...
describe('the shop', () =>
{
    ...
    it('reports stock, price and copy per denomination', async () =>
    {
        ...
        const body = await (await get('/api/pay/catalog')).json() as
            { tiers: Array<{ amount: number; toman: number; available: number; title: string; recommended: boolean }> };
    ...
    it('does not offer a withdrawn denomination at all', async () =>
    {
        ...
        const body = await (await get('/api/pay/catalog')).json() as { tiers: Array<{ amount: number }> };
    ...
    it('refuses an amount that is not a sellable tier', async () =>
    {
        ...
        expect((await post('/api/pay/start', { amount: 7, phone: '09170459330' })).status).toBe(409);
    ...
    it('refuses a payment it could not track, and keeps the code', async () =>
    {
        ...
        fake.requestOverride = { ok: true, authority: 'A-REPEATED', payUrl: 'https://gateway.test/pay/repeat' };
        expect(await buy(10)).toBe(200);
...
describe('the callback', () =>
{
    ...
    async function pending(amount: 5 | 10 | 25 = 10): Promise<{ authority: string; token: string }>
    {
    ...
    it('delivers the code and SMSes it once the payment verifies', async () =>
    {
        ...
        const receipt = await (await get(`/api/pay/receipt?token=${ token }`)).json() as
            { outcome: string; code: string | null; refId: number | null; smsDelivered: boolean; phone: string };
    ...
    it('returns the SAME code on a replayed callback and never takes a second from stock', async () =>
    {
        ...
        await get(`/api/pay/callback?Authority=${ authority }&Status=OK`);
        const first = await (await get(`/api/pay/receipt?token=${ token }`)).json() as { code: string };
        ...
        await get(`/api/pay/callback?Authority=${ authority }&Status=OK`);
        await get(`/api/pay/callback?Authority=${ authority }&Status=OK`);
        const again = await (await get(`/api/pay/receipt?token=${ token }`)).json() as { code: string };
    ...
    it('does not pay out when the gateway refuses to verify, even with Status=OK', async () =>
    {
        ...
        const receipt = await (await get(`/api/pay/receipt?token=${ token }`)).json() as { outcome: string; code: string | null };
    ...
    it('calls a genuine cancellation what it is', async () =>
    {
        ...
        const receipt = await (await get(`/api/pay/receipt?token=${ token }`)).json() as { outcome: string };
    ...
    it('keeps the code when the SMS does not go out', async () =>
    {
        ...
        const receipt = await (await get(`/api/pay/receipt?token=${ token }`)).json() as
            { outcome: string; code: string | null; smsDelivered: boolean };
    ...
    it('records a verified payment it cannot fulfil rather than calling it a failure', async () =>
    {
        ...
        {
            store.startOrder({ id: `drain-${ index }`, amount: 10, toman: PRICES[10], phone: '+989170459330', createdAt: new Date().toISOString() }, 60_000);
        }

        await get(`/api/pay/callback?Authority=${ authority }&Status=OK`);
        const receipt = await (await get(`/api/pay/receipt?token=${ token }`)).json() as { outcome: string; code: string | null; refId: number | null };
        expect(receipt.outcome).toBe('paid');
        expect(receipt.code).toBeNull();
        ...
        const cookie = await post('/api/admin/session', { key: ADMIN_KEY })
            .then((response) => (response.headers.get('set-cookie') ?? '').split(';')[0]);
        const overview = await (await get('/api/admin/overview', { cookie })).json() as { owed: number };
...
describe('the console', () =>
{
    ...
    it('locks out after repeated wrong keys', async () =>
    {
        ...
        {
            expect((await post('/api/admin/session', { key: 'ZZZZ-ZZZZ-ZZZZ-ZZZZ' })).status).toBe(401);
        }
    ...
    it('adds codes, reports duplicates, and hands back what it could not read', async () =>
    {
        ...
        const good = uuids(2);
        const response = await post('/api/admin/codes',
    ...
    it('shows stock and the count that needs a human', async () =>
    {
        ...
        const overview = await (await get('/api/admin/overview', { cookie })).json() as
            { stock: Array<{ amount: number; sold: number }>; owed: number };
    ...
    it('lists the ledger with the buyer, the money and the code', async () =>
    {
        ...
        const page = await (await get('/api/admin/orders', { cookie })).json() as
            { rows: Array<{ phone: string; status: string; code: string | null }>; total: number; page: number };
    ...
    it('finds an order by any of the things a support call starts with', async () =>
    {
        ...
        {
            const found = await (await get(`/api/admin/orders?search=${ encodeURIComponent(term) }`, { cookie })).json() as { total: number };
            return found.total;
        };
    ...
    it('pages the ledger without losing or repeating a row', async () =>
    {
        ...
        const first = await (await get('/api/admin/orders?page=1', { cookie })).json() as
            { rows: Array<{ id: string }>; total: number; pageSize: number };
        const second = await (await get('/api/admin/orders?page=2', { cookie })).json() as { rows: Array<{ id: string }> };
    ...
    it('shows every code and where it went', async () =>
    {
        ...
        const free = await (await get('/api/admin/codes?state=free', { cookie })).json() as { total: number };
        expect(free.total).toBe(1);
        const wrongTier = await (await get('/api/admin/codes?amount=25', { cookie })).json() as { total: number };
        expect(wrongTier.total).toBe(0);

        // And a code is findable by the buyer's number or by part of the code itself.
        const byPhone = await (await get('/api/admin/codes?search=912123', { cookie })).json() as { rows: Array<{ code: string }> };
        expect(byPhone.rows).toHaveLength(1);
        expect(byPhone.rows[0].code).toBe(sold?.code);
        const byCode = await (await get(`/api/admin/codes?search=${ loaded[0].slice(0, 8) }`, { cookie })).json() as { total: number };
...
describe('runtime settings', () =>
{
    ...
    {
        const cookie = await signedIn();
        await post('/api/admin/settings', { merchantId: 'abcdefgh-1234-5678-9012-ffff00005555' }, { cookie });
    ...
    {
        const cookie = await signedIn();
        await post('/api/admin/settings', { merchantId: 'first-merchant-0000-0000-000012349999' }, { cookie });
    ...
    {
        const cookie = await signedIn();
        await post('/api/admin/settings', { zarinpalBase: 'https://sandbox.zarinpal.com' }, { cookie });
        const view = await (await get('/api/admin/settings', { cookie })).json() as { sandbox: boolean };
    ...
    {
        const cookie = await signedIn();
        await post('/api/admin/settings', { merchantId: 'aaaaaaaa-bbbb-cccc-dddd-eeee00007777' }, { cookie });
    ...
    it('mints a console credential on a fresh install rather than shipping a default', () =>
    {
        ...
        expect(minted).toMatch(/^[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{4}(-[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{4}){3}$/);
    ...
    it('keeps every admin route behind the session', async () =>
    {
        ...
        expect((await post('/api/admin/key', { currentKey: ADMIN_KEY, newKey: 'ABCD-ABCD-ABCD-ABCD' })).status).toBe(401);
...
describe('rotating the admin key', () =>
{
    ...
    it('demands the CURRENT key even from an open session', async () =>
    {
        ...
        const refused = await post('/api/admin/key', { currentKey: 'ZZZZ-ZZZZ-ZZZZ-ZZZZ', newKey: NEW_KEY }, { cookie });
    ...
    {
        const cookie = await signedIn();
        expect((await post('/api/admin/key', { currentKey: ADMIN_KEY, newKey: 'short' }, { cookie })).status).toBe(409);
    });

    it('replaces the key, kills every session, and stores only a hash', async () =>
    {
        const cookie = await signedIn();
        expect((await post('/api/admin/key', { currentKey: ADMIN_KEY, newKey: NEW_KEY }, { cookie })).status).toBe(204);
